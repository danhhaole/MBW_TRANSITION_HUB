      ...
      <div>
        ...
        <div v-if="currentStep === 2" class="space-y-4">
          ...
          <div v-if="syncProgress.isActive" class="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
            ...
            <!-- Latest Segment -->
            <div v-if="syncProgress.latestSegment" class="bg-gray-50 rounded-lg p-3">
              <div class="flex items-center gap-2 text-sm">
                <FeatherIcon 
                  :name="syncProgress.latestSegment.action === 'created' ? 'plus-circle' : 'refresh-cw'" 
                  class="h-4 w-4 text-blue-600" 
                />
                <span class="text-gray-600">{{ __('Latest:') }}</span>
                <span class="font-medium text-gray-900">{{ syncProgress.latestSegment.title }}</span>
                <span class="text-xs text-gray-500">({{ syncProgress.latestSegment.action }})</span>
              </div>
            </div>
          </div>
...
if ($socket) {
  $socket.on('position_sync_progress', (data) => {
    if (syncProgress.value.isActive && data.sync_log_name === syncProgress.value.syncLogName) {
      ...
      if (data.segment_title) {
        syncProgress.value.latestSegment = {
          name: data.segment_name,
          title: data.segment_title,
          action: data.action
        }
      }
    }
  ...
  $socket.on('position_sync_complete', (data) => {
    if (syncProgress.value.isActive && data.sync_log_name === syncProgress.value.syncLogName) {
      ...
      if (data.status === 'Completed') {
        showSuccess(__('Sync completed successfully! {0} segments synced.', [data.success_count]))
