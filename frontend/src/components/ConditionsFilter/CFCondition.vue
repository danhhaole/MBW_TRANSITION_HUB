...
import { computed, defineEmits, h, ref, inject } from 'vue'
...
import { filterableFieldsResource } from './filterableFields'
...
const customFieldsInjected = inject('customFields', null)

// Use custom fields or fallback to resource
const filterableFields = computed(() => {
  if (customFieldsInjected && customFieldsInjected.length > 0) {
    return { data: customFieldsInjected }
  }
  return filterableFieldsResource
})
...
function getValueControl() {
  ...
  const fieldData = filterableFields.value?.data?.find((f) => f.fieldname == field)
  if (!fieldData) return null
  const { fieldtype, options, fieldname } = fieldData
  
  // Special handling for Skills and Tags fields - CHECK THIS FIRST!
  if (fieldname === 'skills') {
    return h(SkillsTagsInput, {
      modelValue: props.condition[2],
      placeholder: 'Enter skills separated by commas (e.g. React, Vue, Python)',
      chipColor: 'blue',
  ...
  // Then check operator-based controls
  if (operator == 'is') {
