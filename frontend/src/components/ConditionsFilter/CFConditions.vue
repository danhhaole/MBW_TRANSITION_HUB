...
const filterableFields = computed(() => {
  if (customFieldsInjected && customFieldsInjected.length > 0) {
    return { 
      data: customFieldsInjected,
      submit: () => {} // Mock submit for compatibility
    }
  }
...
watch(
  () => props.doctype,
  (doctype) => {
    // Only submit if using resource (not custom fields)
    if (!customFieldsInjected && filterableFields.value?.submit) {
      filterableFields.value.submit({
        doctype,
      })
    }
  },
