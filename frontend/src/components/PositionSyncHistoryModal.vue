<template>
  <Dialog
    ...
    :options="{
      size: '5xl',
      title: __('Position Sync History')
    }"
  >
    <template #body-content>
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h4 class="font-medium text-gray-900">{{ __('Position to Segment Sync History') }}</h4>
            <p class="text-sm text-gray-500 mt-1">
              {{ __('View all sync operations from ATS to Segment Pool') }}
            </p>
          </div>
          <Button
            variant="outline"
        ...
        <div v-else class="text-center py-12 border rounded-lg bg-gray-50">
          ...
          <p class="text-sm text-gray-400">
            {{ __('Position sync operations will appear here once they are executed.') }}
          </p>
        </div>
...
const fetchSyncLogs = async (page = 1) => {
  ...
  try {
    ...
    console.error('Error fetching position sync logs:', error)
    showError(__('Failed to load position sync history'))
  } finally {
...
const cancelSync = async (syncLogName) => {
  ...
  try {
    ...
    console.error('Error cancelling position sync:', error)
    showError(error.message || __('Failed to cancel position sync'))
  } finally {
...
const retrySync = async (syncLogName) => {
  ...
  try {
    ...
    console.error('Error retrying position sync:', error)
    showError(error.message || __('Failed to retry position sync'))
  } finally {
    actionLoading.value[syncLogName] = false
  }
}
