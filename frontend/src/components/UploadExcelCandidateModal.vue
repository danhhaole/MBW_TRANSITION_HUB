    ...
    <Dialog v-model="show" :options="{ size: '5xl' }">
        <template #body>
            ...
            <div class="flex items-center justify-between p-5 border-b">
                <div class="flex-1">
                    <h3 class="text-xl font-semibold mb-3">{{ __("Upload Candiate") }}</h3>
            ...
            <div class="candidate-uploader">
                ...
                <div v-if="currentStep === 'upload'" class="step-content">
                    <div class="step-header">
                        ...
                        <p class="step-description">{{ __("Download the template and prepare your job opening data") }}
                        </p>
                    </div>
...
const downloadTemplate = async () => {
    ...
    try {
        const response = await call('mbw_mira.mbw_mira.doctype.mira_importsession.mira_importsession.download_candidate_template')
...
const autoMapFields = () => {
    ...
    const commonMappings = {
        'full_name': ['full_name', 'name', 'candidate_name', 'full name', 'họ và tên', 'tên đầy đủ', 'hoten', 'ho ten', 'ho_ten'],
        'email': ['email', 'email_address', 'email address', 'địa chỉ email', 'email_ca_nhan', 'email ca nhan'],
        'phone': ['phone', 'phone_number', 'phone number', 'số điện thoại', 'sdt', 'dien_thoai', 'dien thoai', 'mobile', 'tel'],
        'avatar': ['avatar', 'profile_picture', 'profile picture', 'ảnh đại diện', 'hình đại diện', 'anh_dai_dien'],
        'headline': ['headline', 'title', 'current_title', 'current title', 'chức danh hiện tại', 'tiêu đề', 'tieu_de'],
        'source': ['source', 'source_type', 'nguồn', 'nguồn ứng viên', 'nguon_ung_vien'],
        'cv_original_url': ['cv_url', 'cv_link', 'resume_url', 'resume_link', 'cv', 'resume', 'đường dẫn cv', 'link_cv', 'link cv'],
        'skills': ['skills', 'kỹ năng', 'ky nang', 'ky_nang', 'skill', 'kỹ năng chuyên môn'],
        'ai_summary': ['ai_summary', 'summary', 'tổng quan', 'tom_tat', 'tổng kết', 'tieu_chuan', 'tiêu chuẩn'],
        'status': ['status', 'trạng thái', 'trang_thai', 'tình trạng', 'tinh_trang'],
        'last_interaction': ['last_interaction', 'last_contact', 'ngay_tuong_tac_cuoi', 'ngày tương tác cuối'],
        'email_opt_out': ['email_opt_out', 'unsubscribe', 'khong_nhan_email', 'không nhận email', 'opt_out']
    };
...
const goToCanList = () => {
    closeModal()
    router.push({
        name: 'CandidateManagementSimple',
