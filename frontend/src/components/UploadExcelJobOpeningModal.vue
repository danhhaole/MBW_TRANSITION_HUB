                  ...
                  <div class="flex items-center space-x-4">
                      <div v-for="(step, index) in steps" :key="step.key"
                          ...
                          <span class="ml-2 text-sm font-medium"
                                :class="getStepTextClass(step.key)">
                              {{ __(step.label) }}
                          </span>
          ...
          <div class="candidate-uploader">
              ...
              <div v-if="currentStep === 'upload'" class="step-content">
                  ...
                  <div class="card mb-6">
                      <div class="card-header">
                          <div class="flex items-center">
                              ...
                              <button class="btn btn-outline btn-sm" @click="downloadTemplate" :disabled="isDownloading">
                  ...
                  <!-- <div class="card mb-6">
                      <div class="card-header">
                          <div class="flex items-center">
                              <div class="icon-circle bg-green-100 text-green-600 mr-3">
                                  <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2-2v2m8 0V6a2 2 0 012 2v6M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M8 6v6a2 2 0 002 2h4a2 2 0 002-2V6" />
                                  </svg>
                              </div>
                              <div>
                      ...
                      <div class="card-body">
                          <Link :doctype=""Mira Job Opening"" v-model="selectedJob" :placeholder="__('Select job opening')" />
                      </div>
                  </div> -->
                  ...
                  <div class="card">
                      ...
                      <div class="card-body">
                          <div class="drop-zone" :class="{ 'drag-over': isDragOver }"
                               ...
                               @dragleave.prevent="isDragOver = false" @dragenter.prevent>
              ...
              <div v-if="currentStep === 'mapping'" class="step-content">
                          ...
                          <div class="flex items-center justify-between">
                              <div class="flex items-center">
                                  ...
                                  <div>
                                      ...
                                      <p class="card-subtitle">{{ previewData.total_rows }} {{ __("records found") }}</p>
                                  </div>
                              </div>
                      ...
                      <div class="card-body">
                          <div class="overflow-x-auto max-h-64 border rounded-lg">
                              <table class="min-w-full border-collapse">
                                  ...
                                  <tbody>
                                      <tr v-for="(row, index) in previewData.sample" :key="index" class="hover:bg-gray-50">
                  ...
                  <div class="flex justify-end">
                      <div class="flex space-x-3">
                          <button v-if="canProcess" class="btn btn-outline" @click="validateData" :disabled="isValidating">
                          ...
                          <button class="btn btn-primary" @click="processImport" :disabled="!canProcess || isImporting">
              ...
              <div v-if="currentStep === 'results'" class="step-content">
                      ...
                      <div class="card-body">
                          <div class="max-h-96 overflow-y-auto space-y-2">
                              <div v-for="(log, index) in filteredLogs" :key="index"
                                   class="log-entry" :class="getLogEntryClass(log.status)">
                  ...
                  <div class="flex justify-between">
                      ...
                      <div class="flex space-x-3">
                          ...
                          <button v-if="validateOnly && importResults.success > 0" class="btn btn-success" @click="proceedWithImport">
                              <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                              </svg>
      ...
      <template #body>
          ...
          <div class="p-6">
              ...
              <div v-else class="space-y-4">
                  <div v-for="session in importHistory" :key="session.name" class="border rounded-lg p-4 hover:bg-gray-50">
                      <div class="flex justify-between items-start mb-3">
                          <div class="flex-1">
                              <div class="flex items-center space-x-2 mb-1">
                                  <h4 class="font-medium">{{ session.file_name }}</h4>
                                  <span v-if="session.is_retry" class="px-2 py-1 bg-orange-100 text-orange-800 text-xs rounded-full">{{ __("RETRY") }}</span>
                              </div>
                              <p class="text-sm text-gray-600">{{ __("By") }} {{ session.created_by }} • {{ formatDate(session.started_at) }}</p>
                              <p v-if="session.job_opening" class="text-sm text-blue-600">{{ __("Job:") }} {{ session.job_opening }}</p>
                          </div>
                          <span class="status-badge" :class="getSessionStatusClass(session.status)">{{ session.status }}</span>
                      </div>
                      ...
                      <div class="flex justify-between items-center">
                          ...
                          <div class="flex space-x-2">
                              ...
                              <button v-if="session.status !== 'Processing'" class="btn btn-outline btn-sm text-red-600 border-red-300 hover:bg-red-50" @click="deleteSession(session.name)">
                                  <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const autoMapFields = () => {
  ...
  const commonMappings = {
      'job_title': ['job_title', 'title', 'position', 'role', 'chức danh', 'vị trí', 'tên công việc'],
      'job_code': ['job_code', 'code', 'mã công việc', 'mã job'],
      'description': ['description', 'job_description', 'mô tả', 'mô tả công việc'],
      'requirements': ['requirements', 'requirement', 'yêu cầu', 'điều kiện'],
      'benefits': ['benefits', 'phúc lợi', 'lợi ích', 'chế độ'],
      'department_name': ['department', 'department_name', 'phòng ban', 'bộ phận'],
      'location_name': ['location', 'location_name', 'địa điểm', 'nơi làm việc', 'khu vực'],
      'owner_id': ['owner', 'owner_id', 'người phụ trách', 'quản lý'],
      'number_of_openings': ['number_of_openings', 'slots', 'headcount', 'số lượng', 'số lượng tuyển'],
      'posting_date': ['posting_date', 'start_date', 'ngày đăng tuyển', 'ngày bắt đầu'],
      'closing_date': ['closing_date', 'end_date', 'hạn nộp', 'ngày kết thúc'],
      'approval_status': ['approval_status', 'status', 'trạng thái duyệt', 'tình trạng'],
      'total_applicants': ['total_applicants', 'ứng viên', 'tổng ứng viên', 'số ứng viên'],
      'image': ['image', 'ảnh', 'hình ảnh'],
      'slug': ['slug', 'đường dẫn', 'link'],
      'meta_title': ['meta_title', 'seo_title', 'tiêu đề seo'],
      'meta_description': ['meta_description', 'seo_description', 'mô tả seo'],
      'open_graph_title': ['open_graph_title', 'og_title', 'facebook_title'],
      'open_graph_description': ['open_graph_description', 'og_description', 'facebook_description'],
  }
...
const downloadErrorLog = () => {
  ...
  const csvContent = [
      ['Row Number', 'Job Title', 'Error Message'],
      ...errorLogs.map(log => [
          log.row_number || '',
          log.candidate_name || '',
  ...
  const link = document.createElement('a')
  link.href = url
  link.download = `job_import_errors_${new Date().toISOString().split('T')[0]}.csv`
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
