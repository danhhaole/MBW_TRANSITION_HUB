...
const CAMPAIGN_TYPE = CAMPAIGN_TYPES.ATTRACTION
...
const getActionTypesForTrigger = (triggerType) => {
  if (triggerType === 'ON_INACTIVITY_TIMEOUT') {
    return actionTypeOptions.filter(action => ['EMAIL', 'MESSAGE', 'STOP_TRACKING'].includes(action.value));
  }
...
const availableTriggerTypes = computed(() => {
  ...
  const hiddenTriggers = ['ON_INACTIVITY_TIMEOUT']
  ...
  return triggerTypeOptions.filter(option => {
    ...
    // Filter out hidden triggers for this campaign type
    if (hiddenTriggers.includes(option.value)) return false
