<template>
  <Dialog v-model="show" :options="{ size: '3xl', title: __('Candidate Pool Details') }">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
  ...
  <div class="space-y-6">
    <!-- Basic Information -->
    <!-- Applicant Information (theo Doctype) -->
<div class="bg-white border border-gray-200 rounded-lg p-6">
  ...
  <div class="space-y-3">
    <div class="flex justify-between py-2 border-b border-gray-100">
      ...
      <span class="font-medium">{{ candidatePool.full_name || '-' }}</span>
    </div>
    <div class="flex justify-between py-2 border-b border-gray-100">
      <span class="text-gray-600">{{ __('Email') }}:</span>
      <span class="font-medium">{{ candidatePool.email || '-' }}</span>
    </div>
    <div class="flex justify-between py-2 border-b border-gray-100">
      <span class="text-gray-600">{{ __('Phone') }}:</span>
      <span class="font-medium">{{ candidatePool.phone || '-' }}</span>
    </div>
    <div class="flex justify-between py-2 border-b border-gray-100">
      <span class="text-gray-600">{{ __('Status') }}:</span>
      <span class="font-medium">{{ candidatePool.status || '-' }}</span>
    </div>
    <div class="flex justify-between py-2 border-b border-gray-100">
      <span class="text-gray-600">{{ __('Company') }}:</span>
      <span class="font-medium">{{ candidatePool.company || '-' }}</span>
    </div>
    <div class="flex justify-between py-2 border-b border-gray-100">
      <span class="text-gray-600">{{ __('Location') }}:</span>
      <span class="font-medium">{{ candidatePool.location || '-' }}</span>
    </div>
    <div class="flex justify-between py-2">
      <span class="text-gray-600">{{ __('Email Opt Out') }}:</span>
      <span class="font-medium">
        {{ candidatePool.email_opt_out ? __('Yes') : __('No') }}
      </span>
    </div>
  </div>
</div>
