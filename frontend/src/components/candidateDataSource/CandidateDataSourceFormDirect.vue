...
const loadFieldLayout = async () => {
  ...
  try {
    const response = await candidateDataSourceDirectService.getFormData(props.dataSource?.name || null)
...
const handleSubmit = async () => {
  ...
  try {
    let result
    if (isEdit.value) {
      result = await candidateDataSourceDirectService.update(props.dataSource.name, formData)
    } else {
...
const handleTestConnection = async () => {
  ...
  try {
    const result = await candidateDataSourceDirectService.testConnection(props.dataSource.name)
