<template>
	...
	>
		<Popover class="w-full" v-model:show="showOptions" ref="rootRef" :placement="placement">
						...
						>
							...
							>
								...
								>
									...
									>
										<div
											class="flex flex-1 gap-2 overflow-hidden items-center"
										>
											<div
...
const sanitizeOptions = (options: AutocompleteOption[]) => {
	...
	return options.map((option) => {
		return isOption(option) ? option : { label: String(option), value: option };
	});
};
...
const isSame = (a: AutocompleteOption, b: AutocompleteOption) => {
	const va = isOption(a) ? a.value : a;
	const vb = isOption(b) ? b.value : b;
	return va === vb;
};
...
const selectedValue = computed({
	get() {
		...
		if (!props.multiple) {
			return isOption(props.modelValue)
				? props.modelValue
		...
		return values.map((v) => (isOption(v) ? v : makeOption(v)));
	},
	set(val) {
		query.value = "";
		...
		// Normalize đầu vào và remove Proxy nếu có
		const normalized = (Array.isArray(val) ? val : []).map(toRaw);

		// Đếm số lần xuất hiện của mỗi value
		const countMap = normalized.reduce(
			(acc, curr) => {
				const key = isOption(curr) ? curr.value : curr;
				acc[key] = (acc[key] || 0) + 1;
				return acc;
			},
		...
		// Loại bỏ những thằng xuất hiện nhiều hơn 1 lần
...
const clearAll = () => {
	selectedValue.value = props.multiple ? [] : ("" as string | number | AutocompleteOption);
};
