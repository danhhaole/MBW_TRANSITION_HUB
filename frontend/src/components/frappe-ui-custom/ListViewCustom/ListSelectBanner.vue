...
let selectedText = computed(() => {
  let title =
    list.value.selections.size === 1
      ? __('Row') // Chuyển ngữ cho "Row"
      : __('Rows'); // Chuyển ngữ cho "Rows"
  return `${list.value.selections.size} ${title} ${__('selected')}`; // Chuyển ngữ cho "selected"
});
</script>
