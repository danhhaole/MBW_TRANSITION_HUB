...
export function getScript(doctype, view = 'Form') {
  const scripts = createListResource({
    ...
    onError: (err) => {
      console.error(
        `Error loading Mira Form Scripts for ${doctype} (view: ${view}):`,
        err,
      )
    },
  })
      ...
      Object.defineProperty(FormClass.prototype, 'actions', {
        ...
        get() {
          if (!this._originalDocumentContext) {
            console.warn(
              'Mira Script: _originalDocumentContext not found on instance for actions getter.',
            )
            return []
          }
        ...
        set(newValue) {
          if (!this._originalDocumentContext) {
            console.warn(
              'Mira Script: _originalDocumentContext not found on instance for actions setter.',
            )
            return
          }
          if (!Array.isArray(newValue)) {
            console.warn(
              'Mira Script: "actions" property must be an array. Value was not set.',
              newValue,
            )
    ...
    if (
      ...
      Object.defineProperty(FormClass.prototype, 'statuses', {
        ...
        get() {
          if (!this._originalDocumentContext) {
            console.warn(
              'Mira Script: _originalDocumentContext not found on instance for statuses getter.',
            )
            return []
          }
        ...
        set(newValue) {
          if (!this._originalDocumentContext) {
            console.warn(
              'Mira Script: _originalDocumentContext not found on instance for statuses setter.',
            )
            return
          }
          if (!Array.isArray(newValue)) {
            console.warn(
              'Mira Script: "statuses" property must be an array. Value was not set.',
              newValue,
            )
