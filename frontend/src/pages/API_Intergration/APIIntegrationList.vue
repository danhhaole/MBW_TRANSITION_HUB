  ...
  <div class="external-connections-container">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-600 mt-1">{{__('Manage your connections to external platforms')}}</p>
      </div>
      <Button 
        ...
        <div class="p-4 border-b border-gray-100">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">{{ typeData.platforms.length }} {{__('platforms')}}</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 space-y-3">
          ...
          >
            ...
            <div class="flex items-center justify-between mb-2">
              ...
              <Badge 
                :variant="getConnectionBadgeVariant(platform.id)" 
                size="sm"
              >
            ...
            <!-- Connected Accounts -->
            <div v-if="getConnectedAccountsForPlatform(platform.id).length > 0" class="mb-3">
              <div class="text-xs text-gray-600 mb-1">{{__('Connected Accounts:')}}</div>
              <div class="space-y-1">
                <div 
                ...
                >
                  ...
                  <Badge v-if="account.is_primary" variant="subtle" size="xs">{{__('Primary')}}</Badge>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              ...
              <Button 
                ...
                :disabled="syncingPlatforms.includes(platform.id) || disconnectingPlatforms.includes(platform.id)"
              >
                <template #prefix>
    ...
    >
      <template #body-content>
        <div v-if="editingConnection && selectedPlatform" class="space-y-4">
          ...
          <div v-if="getConnectionStatus(selectedPlatform.id) === 'Pending'" class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <div class="flex items-center gap-2">
              <FeatherIcon name="clock" class="w-4 h-4 text-yellow-600" />
              <span class="text-sm text-yellow-800">
                {{__('This connection is pending authentication. You can edit settings or disconnect if needed.')}}
              </span>
            </div>
          </div>

          <div v-else-if="getConnectionStatus(selectedPlatform.id) === 'Failed'" class="bg-red-50 border border-red-200 rounded-lg p-3">
            <div class="flex items-center gap-2">
              <FeatherIcon name="alert-triangle" class="w-4 h-4 text-red-600" />
              <span class="text-sm text-red-800">
                {{__('This connection failed to authenticate. You can edit settings and try again.')}}
              </span>
            </div>
          </div>

          <div v-else-if="getConnectionStatus(selectedPlatform.id) === 'Disconnected'" class="bg-gray-50 border border-gray-200 rounded-lg p-3">
            <div class="flex items-center gap-2">
              <FeatherIcon name="unlink" class="w-4 h-4 text-gray-600" />
              <span class="text-sm text-gray-800">
                {{__('This connection has been disconnected. You can edit settings to reconnect.')}}
              </span>
            </div>
          </div>

          <!-- Connected Accounts Section - Enhanced -->
          <div v-if="editingConnection.accounts && editingConnection.accounts.length > 0" class="bg-blue-50 rounded-lg p-4">
          ...
          <div v-else-if="getConnectionStatus(selectedPlatform.id) === 'Pending'" class="bg-yellow-50 rounded-lg p-4">
            <div class="flex items-center gap-2">
              <FeatherIcon name="clock" class="w-4 h-4 text-yellow-600" />
              <span class="text-sm text-yellow-800">
                {{__('No accounts connected yet. Complete authentication to see connected accounts.')}}
              </span>
            </div>
          </div>
      ...
      <template #body-content>
        <div v-if="selectedConnection" class="space-y-6">
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            <h3 class="font-semibold text-gray-900 mb-2">{{__('Connection Information')}}</h3>
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span class="text-gray-600">{{__('Platform:')}}</span>
                <span class="ml-2 font-medium">{{ selectedConnection.platform_type }}</span>
              </div>
              <div>
                <span class="text-gray-600">{{__('Status:')}}</span>
                <Badge :variant="selectedConnection.connection_status === 'Connected' ? 'success' : getStatusVariant(selectedConnection.connection_status)" size="sm" class="ml-2">
              ...
              <div>
                ...
                <span class="ml-2">{{ formatDate(selectedConnection.last_sync_time) }}</span>
              </div>
            </div>
          </div>

          <!-- Connected Accounts -->
          <div v-if="selectedConnection.accounts && selectedConnection.accounts.length > 0">
            <h3 class="font-semibold text-gray-900 mb-3">{{__('Connected Accounts')}}</h3>
            <div class="space-y-2">
              <div 
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div class="text-xs text-gray-500">{{ account.account_type }} • {{ account.external_account_id }}</div>
                  </div>
                </div>
                <div class="flex items-center gap-2">
          ...
          <!-- Statistics -->
          <div v-if="selectedConnection.accounts && selectedConnection.accounts.length > 0">
            <h3 class="font-semibold text-gray-900 mb-3">{{__('Connection Statistics')}}</h3>
            <div class="grid grid-cols-3 gap-4">
              <div class="text-center p-3 bg-blue-50 rounded-lg">
                <div class="text-2xl font-bold text-blue-600">{{ selectedConnection.total_api_calls || 0 }}</div>
                <div class="text-xs text-blue-600">{{__('Total API Calls')}}</div>
              </div>
              <div class="text-center p-3 bg-green-50 rounded-lg">
                <div class="text-2xl font-bold text-green-600">{{ selectedConnection.successful_calls || 0 }}</div>
                <div class="text-xs text-green-600">{{__('Successful Calls')}}</div>
              </div>
              <div class="text-center p-3 bg-red-50 rounded-lg">
                <div class="text-2xl font-bold text-red-600">{{ selectedConnection.failed_calls || 0 }}</div>
            ...
            <div class="mt-3 p-3 bg-gray-50 rounded-lg">
              <div class="flex items-center justify-between text-sm">
                ...
                <span class="font-medium" :class="getSuccessRateColor(selectedConnection)">
    ...
    platforms: [
      {
        ...
        features: [__('Pages Management'), __('Posts Publishing'), __('Comments Monitoring'), __('Insights Analytics')]
      }
    ]
  },
  messaging: {
    ...
    platforms: [
      {
        ...
        features: [__('Message Broadcasting'), __('Customer Chat'), __('Template Messages'), __('User Management')]
      }
    ]
  },
  calendar: {
    ...
    platforms: [
      {
        ...
        features: [__('Event Scheduling'), __('Calendar Sync'), __('Booking Management'), __('Availability Settings')]
      }
    ]
  },
  job_board: {
    ...
    platforms: [
      {
        ...
        features: [__('Job Posting'), __('Candidate Management'), __('Application Tracking'), __('Resume Database Access')]
      }
    ]
  }
}
...
const getConnectionByPlatformId = (platformId) => {
  ...
  return connections.value.find(conn => 
    conn.platform_type.toLowerCase() === platformId
  )
}
...
const getModalTitle = (type) => {
  ...
  if (type === 'connect') {
    ...
    return `${__('Edit')} ${selectedPlatform.value.name} ${__('Connection')}${status ? ` (${__(status)})` : ''}`
  }
...
const handleApiResponse = (response, successMessage = __('Operation completed successfully')) => {
...
//   const config = {
//     success: { icon: 'check', iconClasses: 'text-green-600' },
//     error: { icon: 'x', iconClasses: 'text-red-600' },
//     warning: { icon: 'alert-triangle', iconClasses: 'text-orange-600' },
//     info: { icon: 'info', iconClasses: 'text-blue-600' }
//   }
...
//     icon: config[type]?.icon || 'info',
    // iconClasses: config[type]?.iconClasses || 'text-gray-600'
  // })
// }
...
const editConnectionForPlatform = async (platformId) => {
  ...
  try {
    ...
    if (connection.connection_status === 'Connected') {
      try {
    const requestParams = {}
        console.log("Calling API with params:", requestParams)
    const response = await call('mbw_mira.api.external_connections.get_account_details', {
        ...
        console.warn(__('Failed to load account details, proceeding without accounts:'), accountError)
      }
    }
    ...
    showToast('Error', 'Failed to load connection details for editing')
  }
}
...
const openAccountSettingsForPlatform = async (platformId) => {
  ...
  if (connection.connection_status !== 'Connected') {
    showToast('Warning', 'Cannot Access Settings', 'Account settings are only available for connected platforms')
    return
  }
...
const createConnection = async (connectionData) => {
  try {
    ...
    showToast('Error', error.message || 'Failed to create connection')
    throw error
  }
}
...
const updateConnection = async (connectionData) => {
  try {
    ...
    showToast('Error', error.message || 'Failed to update connection')
    throw error
  }
}
...
const confirmAndDisconnect = async (platformId) => {
  ...
  const platformName = Object.values(CONNECTION_TYPES)
    .flatMap(type => type.platforms)
    .find(platform => platform.id === platformId)?.name || __('Platform')
  
  if (!confirm(`${__('Are you sure you want to disconnect from')} ${platformName}? ${__('This action cannot be undone.')}`)) {
    return
  }
...
const refreshSingleConnection = async (connection) => {
  ...
  try {
    ...
    showToast('Success', 'Connection refreshed successfully')
    ...
    showToast('Error', error.message || 'Failed to refresh connection')
  } finally {
...
const refreshConnections = async () => {
  ...
  try {
    ...
    console.log("Calling API with params:", requestParams)
    const response = await call('mbw_mira.api.external_connections.get_connection_info', requestParams)
    ...
    connections.value = []
    showToast('Error', `${__('Failed to refresh connections:')} ${error.message || error}`)
  } finally {
...
const disconnectPlatform = async (platformId) => {
  ...
  if (!canManageConnection(platformId)) {
    showToast('Warning', 'Cannot Disconnect', 'This connection cannot be disconnected in its current state')
    return
  }
  ...
  try {
    const requestParams = {}
    console.log("Calling API with params:", requestParams)
    const response = await call('mbw_mira.api.external_connections.disconnect_connection', {
    ...
    showToast('Error', error.message || 'Failed to disconnect platform')
  } finally {
...
const syncAccounts = async (platformId) => {
  ...
  if (connection.connection_status !== 'Connected') {
    showToast('Warning', 'Cannot Sync', 'Account sync is only available for connected platforms')
    return
  }
  ...
  try {
    ...
    showToast('Error', error.message || 'Failed to sync accounts')
  } finally {
...
const retryConnection = async (platformId) => {
  ...
  if (!isConnectionFailed(platformId)) {
    showToast('Warning', 'Cannot Retry', 'Connection retry is only available for failed connections')
    return
  }
  ...
  try {
    ...
    const result = handleApiResponse(response, __('Connection retry initiated successfully'))
    
    showToast(result.requiresAuth ? 'Retry Initiated' : 'Success', result.message)
    ...
    showToast('Error', error.message || 'Failed to retry connection')
  } finally {
...
const handleConnectionError = async (error, platformId, operation) => {
  ...
  if (error.message && error.message.includes('authentication')) {
    showToast('Warning', 'Authentication Required', 
      'Your authentication has expired. Please reconnect to continue.')
    ...
    showToast('Error', error.message || `${__('Failed to')} ${operation}`)
  }
}
...
onMounted(async () => {
  ...
  setInterval(async () => {
    const hasPendingConnections = connections.value.some(conn => 
