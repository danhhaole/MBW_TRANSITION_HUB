<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <div class="container mx-auto px-6 py-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-3 items-center">
                    <Select v-model="campaignFilter"
                        :options="[{ label: 'All Campaigns', value: '' }, ...campaignOptions.map(c => ({ label: c.label, value: c.value }))]"
                        class="w-48" @update:model-value="handleSearch" />
                    <Select v-model="segmentFilter"
                        :options="[{ label: 'All Segments', value: '' }, ...segmentOptions.map(s => ({ label: s.label, value: s.value }))]"
                        class="w-48" @update:model-value="handleSearch" />
            ...
            <div class="bg-white rounded-lg border border-gray-200">
                ...
                <template v-else-if="items.length === 0">
                    <div class="text-center py-16">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('No applications found') }}</h3>
                ...
                <template v-else>
                    ...
                    <div v-if="pagination.pages > 1"
                        ...
                        <div class="text-sm text-gray-600">
                            {{ __('Display') }} {{ pagination.showing_from || 1 }}-{{ pagination.showing_to ||
                            items.length }} {{ __('of') }} {{ pagination.total || items.length }} {{ __('applications')
                            }}
                        </div>
                        <div class="flex space-x-1">
                            ...
                            <template v-for="page in getPaginationRange()" :key="page">
                                <button v-if="page === '...'" class="px-3 py-1 text-gray-500" disabled>...</button>
            ...
            <!-- Dialogs -->
            <Dialog v-model="showEditDialog" :options="{ title: __('Edit Application'), size: '2xl' }">
                <template #body-content>
                    <div class="space-y-4 w-full">
                        ...
                        <FormControl type="text" label="Resume" v-model="editItem.resume"
                            :description="'File URL hoặc đường dẫn file'" />
                        <FormControl type="select" label="Status" v-model="editItem.application_status" :options="[
                            { label: 'New', value: 'New' },
                        ...
                        <FormControl type="select" label="Result" v-model="editItem.result" :options="[
                ...
                <template #actions>
                    <div class="flex gap-2 justify-end">
                        <Button variant="ghost" @click="showEditDialog = false">{{ __('Cancel') }}</Button>
                        <Button variant="solid" @click="saveEdit">{{ __('Save') }}</Button>
                    </div>
                </template>
            </Dialog>
            <Dialog v-model="showViewDialog" :options="{ title: __('Application Details'), size: '2xl' }">
                <template #body-content>
                    <div class="space-y-4">
                        ...
                        <FormControl type="text" label="Application Position" :model-value="viewItem.position"
                            disabled />
                        <FormControl type="text" label="Resume" :model-value="viewItem.resume" disabled />
                        <FormControl type="text" label="Status" :model-value="viewItem.application_status" disabled />
                        <FormControl type="textarea" label="Notes" :model-value="viewItem.notes" disabled />
                        <FormControl type="number" label="Score" :model-value="viewItem.score" disabled />
                        <FormControl type="text" label="Result" :model-value="viewItem.result" disabled />
                    </div>
                </template>
            </Dialog>
            <Dialog v-model="showDeleteDialog" :options="{ title: __('Confirm Delete'), size: 'sm' }">
                <template #body-content>
                    <div class="text-center">
                        ...
                        <p class="text-slate-600 mb-2">{{ __('Are you sure you want to delete this application?') }}</p>
                        <p class="font-medium text-gray-900">"{{ deleteItem?.name }}"</p>
                        <p class="text-sm text-red-600 mt-2">{{ __('This action cannot be undone!') }}</p>
                    </div>
                </template>
                <template #actions>
                    <Button variant="ghost" @click="showDeleteDialog = false">{{ __('Cancel') }}</Button>
                    <Button variant="solid" theme="red" @click="confirmDelete">{{ __('Delete') }}</Button>
                </template>
            </Dialog>
        </div>
    </div>
...
import { Dialog, Button, Breadcrumbs, Select, FormControl, DateTimePicker } from 'frappe-ui'
import LayoutHeader from '@/components/LayoutHeader.vue'
import Loading from '@/components/Loading.vue'
import Link from '@/components/Controls/Link.vue'
import { applicantPoolService, getCampaignOptions, getSegmentOptions } from '@/services/universalService'
...
async function loadData() {
    ...
    try {
        ...
        pagination.showing_to = Math.min(pagination.page * pagination.limit, pagination.total)
    } finally {
        loading.value = false
    }
}
