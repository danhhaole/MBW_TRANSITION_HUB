<template>
  <div class="candidate-detail-view min-h-screen bg-gray-50">
        ...
        <div class="flex items-center">
          ...
          <div class="flex items-center">
            <Avatar
              ...
              :label="getAvatarText(candidate.candidate_name)"
              size="xl"
              class="mr-4"
            />
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-4">
            <div>
              ...
              <p class="text-gray-900">{{ candidate.candidate_name || 'N/A' }}</p>
            </div>
            <div>
          ...
          <div class="space-y-4">
            <div>
              ...
              <p class="text-gray-900">{{ candidate.position || 'N/A' }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Experience</label>
              <p class="text-gray-900">{{ candidate.experience || 'N/A' }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Location</label>
              <p class="text-gray-900">{{ candidate.location || 'N/A' }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Created</label>
              <p class="text-gray-900">{{ formatDate(candidate.creation) || 'N/A' }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="p-6">
            ...
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else-if="candidateCampaigns.length === 0">
                  <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">No campaigns assigned</td>
                </tr>
                <tr v-else v-for="item in candidateCampaigns" :key="item.name" class="hover:bg-gray-50">
            ...
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else-if="candidateSegments.length === 0">
                  <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">No segments assigned</td>
                </tr>
                <tr v-else v-for="item in candidateSegments" :key="item.name" class="hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
        ...
        <div v-if="activeTab === 'interactions'">
            ...
            <ul role="list" class="-mb-8">
              <li v-for="(interaction, idx) in interactions" :key="interaction.name">
                <div class="relative pb-8">
                  <span v-if="idx !== interactions.length - 1" class="absolute top-4 left-4 -ml-px h-full w-0.5 bg-gray-200" aria-hidden="true"></span>
                  <div class="relative flex space-x-3">
                    <div>
                      <span 
                        class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white"
                        :class="getInteractionBgClasses(interaction.interaction_type)"
                      >
                        <component :is="getInteractionIcon(interaction.interaction_type)" class="w-4 h-4 text-white" />
                      </span>
                    </div>
          ...
          <div v-else class="text-center py-12">
            <svg class="w-12 h-12 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-3.582 8-8 8a8.001 8.001 0 01-7.227-4.612 2.5 2.5 0 00-.673-2.388A8 8 0 0112 4c4.418 0 8 3.582 8 8z"/>
            </svg>
            ...
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else-if="emailLogs.length === 0">
                  <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">No emails found</td>
                </tr>
                <tr v-else v-for="item in emailLogs" :key="item.name" class="hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {{ item.subject }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
        ...
        <div v-if="activeTab === 'documents'">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <div v-for="doc in documents" :key="doc.id" class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              <div class="flex items-center justify-between mb-3">
                <component :is="getDocumentIcon(doc.type)" class="w-8 h-8 text-blue-600" />
                <div class="relative">
                  <button class="p-1 text-gray-400 hover:text-gray-600" @click="toggleDocumentMenu(doc.id)">
              ...
              <h4 class="text-sm font-medium text-gray-900 truncate mb-1">{{ doc.name }}</h4>
...
const getInteractionIcon = (type) => {
  const iconMap = {
    'EMAIL_SENT': {
      render() {
        return h('svg', { class: 'w-4 h-4', fill: 'currentColor', viewBox: '0 0 20 20' }, [
          h('path', { d: 'M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z' }),
    ...
    'EMAIL_CLICKED': {
      render() {
        return h('svg', { class: 'w-4 h-4', fill: 'currentColor', viewBox: '0 0 20 20' }, [
          h('path', { 'fill-rule': 'evenodd', d: 'M12.586 4.586a2 2 0 112.828 2.828l-3 3a2 2 0 01-2.828 0 1 1 0 00-1.414 1.414 4 4 0 005.656 0l3-3a4 4 0 00-5.656-5.656l-1.5 1.5a1 1 0 101.414 1.414l1.5-1.5zm-5 5a2 2 0 012.828 0 1 1 0 101.414-1.414 4 4 0 00-5.656 0l-3 3a4 4 0 105.656 5.656l1.5-1.5a1 1 0 10-1.414-1.414l-1.5 1.5a2 2 0 11-2.828-2.828l3-3z', 'clip-rule': 'evenodd' })
        ])
      }
    }
  }
