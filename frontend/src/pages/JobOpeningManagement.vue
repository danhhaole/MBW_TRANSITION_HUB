  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="container mx-auto px-6 py-6">
      ...
      <Dialog 
        ...
        :options="{ 
          size: '4xl',
          // Chặn đóng modal khi click outside
          closeOnClickOutside: false,
          closeOnEscape: false
        }"
        @close="handleMainModalClose"
      >
        <template #body>
      ...
      >
        <template #body>
          <div class="p-6 relative max-h-[80vh] overflow-y-auto">
            ...
            <button
              @click="closeAIModal"
              class="absolute top-2 right-2 z-10 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors"
              title="Close"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
...
import { getFilteredJobOpenings, submitNewJobOpening, updateJobOpeningData, removeJobOpening } from '@/services/jobOpeningService'
...
const openCreateDialog = () => {
  ...
  currentStep.value = 1
  showForm.value = true
  isMainFormOpen.value = true
  preventCloseMainForm.value = false
}

const handleMainModalClose = () => {
  // Ngăn đóng modal chính bằng cách giữ nguyên showForm = true
  showForm.value = true
  return false
}
...
const handleAIModalClose = () => {
  // Khi đóng modal AI (click outside hoặc ESC), giữ modal tạo job mở
  showAIModal.value = false
  // Modal tạo job vẫn mở
}

const closeForm = () => { 
  showForm.value = false
  currentStep.value = 1
  isMainFormOpen.value = false
  preventCloseMainForm.value = false
}
...
// Thêm biến để theo dõi trạng thái modal
const isMainFormOpen = ref(false)
const preventCloseMainForm = ref(false)

// Sửa hàm mở modal AI
const openAIModal = () => {
  // Đảm bảo modal tạo job vẫn mở
  if (!showForm.value) {
    showForm.value = true
    isMainFormOpen.value = true
  }
  
  // Ngăn modal tạo job đóng khi modal AI mở
  preventCloseMainForm.value = true
...
const closeAIModal = () => {
  showAIModal.value = false
  // Cho phép modal tạo job đóng lại
  preventCloseMainForm.value = false
}
...
// Thêm vào onMounted hoặc watch
watch(showAIModal, (newVal) => {
  if (newVal) {
    // Thêm event listener khi modal AI mở
    document.addEventListener('click', handleOutsideClick)
  } else {
    // Xóa event listener khi modal AI đóng
    document.removeEventListener('click', handleOutsideClick)
  }
})

const handleOutsideClick = (event) => {
  // Kiểm tra nếu click ra ngoài modal AI
  const modalElement = document.querySelector('[data-modal="ai-generation"]')
