    ...
    <div class="h-screen w-full flex flex-col">
        ...
        <div class="bg-white border-b px-4 py-3 flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-lg font-semibold">{{ _template?.template_name || __('Email Template Editor') }}</h1>
        ...
        <template #body-content>
            <div class="space-y-4">
                ...
                <Checkbox
                    ...
                    :label="__('Automatically send email to candidates upon adding them to the system')"
                />
            </div>
        </template>
        <template #actions>
            <div class="flex justify-end space-x-2">
                ...
                <Button variant="solid" theme="blue" @click="handleEditInfoSave" :loading="loading">
                    {{ __('Save') }}
                </Button>
            </div>
        </template>
				...
				<div class="border rounded-lg">
					<div class="bg-gray-50 px-4 py-2 border-b">
						<div class="flex items-center justify-between">
							<span class="text-sm font-medium text-gray-600">{{ __('Email Content') }}</span>
							<div class="flex space-x-2">
								<button 
									@click="previewMode = 'desktop'" 
									:class="previewMode === 'desktop' ? 'bg-blue-100 text-blue-700' : 'text-gray-500'"
								...
								<button 
									...
									:class="previewMode === 'mobile' ? 'bg-blue-100 text-blue-700' : 'text-gray-500'"
									class="px-2 py-1 text-xs rounded"
								>
									📱 Mobile
								</button>
	...
	<Dialog v-model="showSendModal" :options="{ size: '4xl' }">
		...
		<template #body-content>
			<div class="space-y-4">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-2">{{ __('Send To') }}</label>
						<FormControl
							type="email"
				...
				<div class="border rounded-lg p-4 bg-gray-50">
					<h4 class="text-sm font-medium text-gray-700 mb-3">{{ __('Sample Data for Merge Fields') }}</h4>
					<div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-40 overflow-y-auto">
						<div v-for="field in mergeFields" :key="field.name" class="flex items-center space-x-2">
							<label :for="field.name" class="text-xs font-medium text-gray-600 w-24 truncate">
								{{ field.name }}:
							</label>
							<FormControl
								:id="field.name"
				...
				<div class="border rounded-lg">
					<div class="bg-gray-50 px-4 py-2 border-b">
						<div class="flex items-center justify-between">
							<span class="text-sm font-medium text-gray-600">{{ __('Email Preview with Sample Data') }}</span>
							<div class="flex space-x-2">
								<button 
									@click="sendPreviewMode = 'desktop'" 
									:class="sendPreviewMode === 'desktop' ? 'bg-blue-100 text-blue-700' : 'text-gray-500'"
									class="px-2 py-1 text-xs rounded"
								>
									🖥️ Desktop
								</button>
								<button 
									...
									:class="sendPreviewMode === 'mobile' ? 'bg-blue-100 text-blue-700' : 'text-gray-500'"
									class="px-2 py-1 text-xs rounded"
								>
									📱 Mobile
								</button>
		...
		<template #actions>
			<div class="flex justify-end space-x-2">
				...
				<Button variant="solid" theme="purple" @click="sendTestEmail" :loading="sendingEmail">
					{{ __('Send Test Email') }}
				</Button>
			</div>
		</template>
...
const loadMergeFields = async () => {
    try {
        const response = await createResource({
            url: "mbw_mira.mbw_mira.doctype.mira_email_template.mira_email_template.get_merge_fields",
            auto: true,
            onSuccess: (data) => {
