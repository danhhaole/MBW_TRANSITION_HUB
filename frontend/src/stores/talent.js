  ...
  getters: {
    filteredTalents: (state) => {
      ...
      if (state.filters.current_status) {
        filtered = filtered.filter(talent => talent.current_status === state.filters.current_status)
      }
    ...
    async fetchTalents(options = {}) {
      ...
      try {
        const params = {
          doctype: 'Mira Talent',
          fields: [
            'name', 'full_name', 'email', 'phone', 'gender', 'date_of_birth',
            'linkedin_profile', 'facebook_profile', 'zalo_profile',
            'current_location', 'preferred_location', 'skills', 'tags', 'source',
            'experience_years', 'education', 'experience', 'certifications',
            'languages', 'resume', 'current_status', 'notes',
        ...
        if (this.filters.current_status) {
          params.filters = { ...params.filters, current_status: this.filters.current_status }
        }
      ...
      try {
        ...
        try {
          ...
          if (result && result.success && Array.isArray(result.data)) {
            ...
            const avgExperience = talents.reduce((sum, t) => sum + (parseFloat(t.experience_years) || 0), 0) / total || 0
          ...
          // Fallback to old API
          const totalResult = await call('frappe.client.get_count', { doctype: 'Mira Talent' })
