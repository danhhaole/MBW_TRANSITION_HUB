...
@frappe.whitelist()
def create_template_flow_from_trigger(campaign_template_id, trigger):
    """
    Create Mira Flow from campaign template trigger
    
    Args:
        campaign_template_id: Mira Campaign Template ID
    ...
    try:
        ...
        template = frappe.get_doc("Mira Campaign Template", campaign_template_id)
        ...
        flow.title = f"{template.template_name} - {get_trigger_label(trigger.get('trigger_type'))}"
        flow.description = f"Auto-generated flow for campaign template {template.template_name}"
        flow.status = "Active"
        flow.type = "Template"
        flow.campaign_template_id = campaign_template_id
        ...
        frappe.log_error(f"Error creating template flow from trigger: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
@frappe.whitelist()
def get_template_flows(campaign_template_id):
    """
    Get all flows for a campaign template and convert to trigger format
    ...
    """
    try:
        # Get all flows for this campaign template
        flows = frappe.get_all(
            "Mira Flow",
            filters={"campaign_template_id": campaign_template_id},
        ...
        for flow_info in flows:
            ...
            if flow.trigger_id and len(flow.trigger_id) > 0:
                ...
                trigger_type = flow_trigger.trigger_type
        ...
        frappe.log_error(f"Error getting template flows: {str(e)}")
        return {
            "success": False,
            "error": str(e),
            "data": []
...
@frappe.whitelist()
def create_or_update_template_flow(campaign_template_id, trigger):
    """
    Create or update Mira Flow from trigger data for template
    - If flow exists for this template + trigger_type → Update
    ...
    try:
        ...
        # Find existing flow for this template and trigger type
        ...
        if existing_flow_id:
            ...
            flow.title = f"{frappe.get_value('Mira Campaign Template', campaign_template_id, 'template_name')} - {get_trigger_label(trigger_type)}"
            ...
            flow.action_id = []
            for idx, action in enumerate(trigger.get("actions", []), start=1):
                # Use action_parameters from action if provided
                if action.get("action_parameters"):
                    action_parameters = action.get("action_parameters")
                else:
            ...
            # Create new flow
            result = create_template_flow_from_trigger(campaign_template_id, trigger)
        ...
        frappe.log_error(f"Error in create_or_update_template_flow: {str(e)}")
        return {
            "success": False,
            "message": str(e)
        }
...
@frappe.whitelist()
def sync_template_flows(campaign_template_id, triggers):
    """
    Sync all flows for a campaign template in one API call
    ...
    try:
        ...
        for trigger in triggers:
            result = create_or_update_template_flow(campaign_template_id, trigger)
            if result.get("success"):
        ...
        frappe.log_error(f"Error in sync_template_flows: {str(e)}")
        return {
            "success": False,
            "message": str(e)
        }
