...
from mbw_mira.utils.ai import call_llm_convert_json_to_text_JD
...
class MiraSegment(Document):
	...
	def on_update(self):
		try:
			frappe.enqueue(
				"mbw_mira.mbw_mira.doctype.mira_segment.mira_segment.summary_pool_ai",
				queue="default",
				pool_id=self.name
			)
...
def delete_segment(name=None):
    ...
    try:
        ...
        if not name:
            return {"status": "error", "message": _("Segment name is required")}
        ...
        if talent_count > 0:
            return {
                "status": "error",
                "message": _(
                    "Cannot delete segment. Please remove all {0} talent(s) from the pool before deleting the segment."
                ).format(talent_count),
        ...
        frappe.db.rollback()
        frappe.log_error(
            message=frappe.get_traceback(), title=f"Delete Segment Error: {name}"
        )
        ...
        for link_info in linked_doctypes:
            ...
            if doctype == "Mira Pool Vecto" and fieldname == "mira_segment":
                ...
                pool_vecto_docs = frappe.get_all(
                    "Mira Pool Vecto", filters={fieldname: doc.name}, fields=["name"]
                )
...
def check_segment_links(name=None):
    ...
    try:
        ...
        frappe.log_error(
            message=frappe.get_traceback(), title=f"Check Segment Links Error: {name}"
        )
        return {"status": "error", "message": str(e)}
...
def calculate_segment_engagement_rate(segment_id=None, max_expected_score=None):
    ...
    try:
        ...
        if not segment_id:
            return {"status": "error", "message": _("Segment ID is required")}
        ...
        talent_pool = frappe.get_all(
            "Mira Talent Pool", filters={"segment_id": segment_id}, fields=["talent_id"]
        )
        ...
        total_engagement_score = sum(
            interaction.engagement_score or 0 for interaction in interactions
        )

        # Đếm số talent có ít nhất 1 interaction
        talents_with_interactions = len(
            set(interaction.talent_id for interaction in interactions)
        )

        # Tính engagement rate theo 2 công thức
        # Công thức 1: (Số talent có tương tác / Tổng số talent) * 100
        engagement_rate_by_participation = (
            (talents_with_interactions / total_talents * 100)
            if total_talents > 0
            else 0
        )

        # Công thức 2: (Tổng điểm engagement / Tổng số talent) - normalized to percentage
        average_score_per_talent = (
            (total_engagement_score / total_talents) if total_talents > 0 else 0
        )

        # Normalize average score to percentage
        # Use provided max_expected_score or default to 100
        max_expected_score_per_talent = (
            float(max_expected_score) if max_expected_score else 100.0
        )
        engagement_rate_by_score = (
            min(100, (average_score_per_talent / max_expected_score_per_talent * 100))
            if max_expected_score_per_talent > 0
            else 0
        )

        # Combined engagement rate (weighted average of both methods)
        engagement_rate = (
            engagement_rate_by_participation * 0.6 + engagement_rate_by_score * 0.4
        )
        ...
        for interaction in interactions:
            ...
            if interaction_type not in interaction_stats:
                interaction_stats[interaction_type] = {"count": 0, "total_score": 0}
            interaction_stats[interaction_type]["count"] += 1
            interaction_stats[interaction_type]["total_score"] += (
                interaction.engagement_score or 0
            )

        return {
            "status": "success",
            "engagement_rate": round(engagement_rate, 2),
            "engagement_rate_by_participation": round(
                engagement_rate_by_participation, 2
            ),
...
def get_segments_engagement_rates(segment_ids=None):
    ...
    try:
        ...
        if not segment_ids:
            return {"status": "error", "message": _("Segment IDs are required")}
        ...
        if not isinstance(segment_ids, list):
            return {"status": "error", "message": _("Invalid segment IDs format")}
        ...
        for segment_id in segment_ids:
            ...
            if result.get("status") == "success":
                results[segment_id] = {
                    ...
                    "engagement_rate_by_participation": result.get(
                        "engagement_rate_by_participation", 0
                    ),
                    "engagement_rate_by_score": result.get(
                        "engagement_rate_by_score", 0
                    ),
                    "total_talents": result.get("total_talents", 0),
                    "talents_with_interactions": result.get(
                        "talents_with_interactions", 0
                    ),
                    "total_interactions": result.get("total_interactions", 0),
                    "average_score_per_talent": result.get(
                        "average_score_per_talent", 0
                    ),
                }
            else:
        ...
        frappe.log_error(
            message=frappe.get_traceback(), title="Get Segments Engagement Rates Error"
        )
        return {"status": "error", "message": str(e)}


# Tạo summary cho Pool
def summary_pool_ai(pool_id):
    ...
    embeding_json = call_llm_convert_json_to_text_JD(json.dumps(pool))
    if embeding_json:
        frappe.db.set_value(
