...
class MiraViewSettings(Document):
	pass
...
def create(view):
	...
	doc = frappe.new_doc("Mira View Settings")
	doc.name = view.label
...
def update(view):
	...
	doc = frappe.get_doc("Mira View Settings", view.name)
	doc.label = view.label
	doc.type = view.type or "list"
	doc.icon = view.icon
	doc.route_name = view.route_name or get_route_name(view.doctype)
...
@frappe.whitelist()
def delete(name):
	if frappe.db.exists("Mira View Settings", name):
		frappe.delete_doc("Mira View Settings", name)
...
def public(name, value):
	...
	doc = frappe.get_doc("Mira View Settings", name)
	if doc.pinned:
		doc.pinned = False
	doc.public = value
...
def set_as_default(name=None, type=None, doctype=None):
	if name:
		frappe.db.set_value("Mira View Settings", name, "is_default", 1)
	else:
...
def create_or_update_standard_view(view):
	...
	if doc:
		doc = frappe.get_doc("Mira View Settings", doc)
		doc.label = view.label
		doc.type = view.type or "list"
		doc.route_name = view.route_name or get_route_name(view.doctype)
		doc.load_default_columns = view.load_default_columns or False
...
def get_route_name(doctype):
	# Example: "Mira Lead" -> "Leads"
	if doctype.startswith("Mira "):
		doctype = doctype[4:]
