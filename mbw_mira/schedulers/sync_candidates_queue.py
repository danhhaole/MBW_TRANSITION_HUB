...
def run():
    """
    Process pending candidate sync jobs from queue
    """
    ...
    # from mbw_mira.workers.ats.fetch_mbw_ats_data import sync_data_source_candidates_background
    ...
    for sync_job in pending_syncs:
        try:
            ...
            frappe.log_error(f"Failed to enqueue sync job {sync_job.name}: {str(e)}", "Sync Queue Error")
...
def cancel_sync_job(sync_log_name):
    ...
    try:
        ...
        if sync_log.status in ["Pending", "In Progress"]:
            ...
            sync_log.end_time = frappe.utils.now()
            sync_log.details = f"Sync cancelled by user at {frappe.utils.now()}"
            sync_log.save(ignore_permissions=True)
            frappe.db.commit()
            return True
        else:
        ...
        frappe.log_error(f"Failed to cancel sync job {sync_log_name}: {str(e)}", "Cancel Sync Error")
        return False
...
def retry_failed_sync(sync_log_name):
    ...
    try:
        ...
        if sync_log.status in ["Failed", "Partially Completed"]:
            ...
            sync_log.details = f"Retry requested at {frappe.utils.now()}"
            sync_log.save(ignore_permissions=True)
            frappe.db.commit()
            return True
        else:
        ...
        frappe.log_error(f"Failed to retry sync job {sync_log_name}: {str(e)}", "Retry Sync Error")
        return False
